...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"Error: Could not find module '{html.escape(module_name)}'")
        print(f"Details: {html.escape(str(e))}")
        print("\nMake sure the module path is correct (e.g., src.sr.1_train_model)")
        sys.exit(1)
    except KeyboardInterrupt:
        send_telegram_message(
            f"<b>Training interrupted (KeyboardInterrupt)</b>\n\n"
            f"<code>{html.escape(module_name)}</code>\n"
            f"Args: <code>{html.escape(original_args)}</code>"
        )
        sys.exit(130)
        ...
        error_msg = (
            ...
            f"Args: <code>{html.escape(original_args)}</code>\n\n"
            f"<b>Error:</b> {html.escape(type(e).__name__)}: {e}\n\n"
