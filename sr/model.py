...
from typing import Tuple, List, Optional, override, final, Callable
from beartype import beartype

sys.path.insert(0, "../")  # run under the current directory
from common.network import MuLUTConv, MuLUTcUnit, AutoSample, Residual
...
class ConvBlock(nn.Module):
    def __init__(
        ...
        for c in range(in_c):
            for mode in modes:
                module_dict["DepthwiseBlock{}_{}".format(c, mode)] = MuLUTConv(
        ...
        if scale is None:
            self.pixel_shuffle: Callable[[torch.Tensor], torch.Tensor] = identity
        else:
            self.pixel_shuffle: Callable[[torch.Tensor], torch.Tensor] = (
                nn.PixelShuffle(scale)
            )

    def forward(self, x: torch.Tensor, prev_x: list[torch.Tensor] | None):
        modes = self.modes
        ...
        for c in range(self.in_c):
            x_c = x[:, c : c + 1, :, :]
            prevx_c = (
                [px[:, c : c + 1, :, :] for px in prev_x] if prev_x != None else None
            )
            pred = 0
            for mode in modes:
                # pad = mode_pad_dict[mode]
                pad = self.sample_size - 1
                sub_module: torch.nn.Module = self.module_dict[
                    "DepthwiseBlock{}_{}".format(c, mode)
                ]
                for r in [0, 1, 2, 3]:
                    y = round_func(
                        torch.tanh(
...
class SPF_LUT_net(nn.Module):
    def __init__(
        ...
        self.channel_conv1 = MuLUTcUnit(in_c=3, out_c=3, mode="1x1", nf=nf)
    ...
    @override
    def forward(self, x: torch.Tensor, phase: str = "train"):
        B, C, H, W = x.size()
        x = x.reshape((B * C, 1, H, W))
        avg_factor, bias, norm = len(self.modes) * 4, 127, 255.0
        refine_list: list[torch.Tensor] = []
        ...
        x = round_func(torch.tanh(self.channel_conv1(x)) * 127.0)
        x = round_func(torch.clamp(x + 127, 0, 255)) / 255.0

        # block4 input_channel=3, so prev_num=1, cat as channels
        x4 = x
        x = self.convblock4(x, [torch.cat([x1, x2, x3], dim=1)])
        x = round_func(torch.clamp((x / avg_factor) + bias, 0, 255)) / norm
        ...
        # block5
        x5 = x
        x = self.convblock5(x, [x1, x2, x3, x4])
        x = round_func(torch.clamp((x / avg_factor) + bias, 0, 255)) / norm
        ...
        # block6
        x6 = x
        x = self.convblock6(x, [x1, x2, x3, x4, x5])
        x = round_func(torch.clamp((x / avg_factor) + bias, 0, 255)) / norm
...
class SPF_LUT_DFC(nn.Module):
    ...
    def __init__(
        ...
        def load_sampler_and_residual(stage, mode, channel=0):
            ...
            sampler_path = os.path.join(
                lut_folder, "sampler_s{}c{}_{}.npy".format(stage, channel, mode)
            )
            ...
            res_path = os.path.join(
                lut_folder, "residual_s{}c{}_{}.npy".format(stage, channel, mode)
            )
            res_weights = torch.tensor(np.load(res_path))
            self.register_parameter(
                "residual_s{}c{}_{}".format(stage, channel, mode),
                nn.Parameter(res_weights),
            )
        ...
        for mode in modes:
            ...
            if phase == "train":
                lut_path = os.path.join(
                    lut_folder,
                    "{}_x{}_4bit_int8_s{}c0_{}_compress1.npy".format(
                        lutName, upscale, 1, mode
                    ),
                )
            else:
                lut_path = os.path.join(
                    lut_folder, "{}_s{}c0_{}_compress1.npy".format(lutName, 1, mode)
                )
            key = "s{}c0_{}_compress1".format(1, mode)
            if compressed_dimensions == "xy":
                lut_arr = (
                    np.load(lut_path).reshape((-1, L * L, 2)).astype(np.float32) / 127.0
                )
            elif compressed_dimensions == "xyz":
                lut_arr = (
                    np.load(lut_path).reshape((-1, L, 2)).astype(np.float32) / 127.0
                )
            ...
            self.register_parameter(
                name="weight_" + key, param=torch.nn.Parameter(torch.Tensor(lut_arr))
            )

            if phase == "train":
                lut_path = os.path.join(
                    lut_folder,
                    "{}_x{}_4bit_int8_s{}c0_{}_compress2.npy".format(
                        lutName, upscale, 1, mode
                    ),
                )
            else:
                lut_path = os.path.join(
                    lut_folder, "{}_s{}c0_{}_compress2.npy".format(lutName, 1, mode)
                )
            ...
            if phase == "train":
                lut_path = os.path.join(
                    lut_folder,
                    "{}_x{}_4bit_int8_s{}c0_{}_compress1.npy".format(
                        lutName, upscale, 2, mode
                    ),
                )
            else:
                lut_path = os.path.join(
                    lut_folder, "{}_s{}c0_{}_compress1.npy".format(lutName, 2, mode)
                )
            ...
            if phase == "train":
                ...
                lut_path = os.path.join(
                    lut_folder, "{}_s{}c0_{}_compress2.npy".format(lutName, 2, mode)
                )
            ...
            if phase == "train":
                lut_path = os.path.join(
                    lut_folder,
                    "{}_x{}_4bit_int8_s{}c0_{}_compress1.npy".format(
                        lutName, upscale, 3, mode
                    ),
                )
            else:
                lut_path = os.path.join(
                    lut_folder, "{}_s{}c0_{}_compress1.npy".format(lutName, 3, mode)
                )
            ...
            if phase == "train":
                ...
                lut_path = os.path.join(
                    lut_folder, "{}_s{}c0_{}_compress2.npy".format(lutName, 3, mode)
                )
            ...
            if phase == "train":
                lut_path = os.path.join(
                    lut_folder,
                    "{}_x{}_4bit_int8_s{}c0_{}_compress1.npy".format(
                        lutName, upscale, 4, mode
                    ),
                )
            else:
                lut_path = os.path.join(
                    lut_folder, "{}_s{}c0_{}_compress1.npy".format(lutName, 4, mode)
                )
            key = "s{}c0_{}_compress1".format(4, mode)
            if compressed_dimensions == "xy":
                lut_arr = (
                    np.load(lut_path).reshape((-1, L * L, 1)).astype(np.float32) / 127.0
                )
            elif compressed_dimensions == "xyz":
                lut_arr = (
                    np.load(lut_path).reshape((-1, L, 1)).astype(np.float32) / 127.0
                )
            elif compressed_dimensions == "xyzt":
            ...
            if phase == "train":
                ...
                lut_path = os.path.join(
                    lut_folder, "{}_s{}c0_{}_compress2.npy".format(lutName, 4, mode)
                )
            ...
            for c in range(4):
                ...
                if phase == "train":
                    lut_path = os.path.join(
                        lut_folder,
                        "{}_x{}_4bit_int8_s{}c{}_{}_compress1.npy".format(
                            lutName, upscale, 6, c, mode
                        ),
                    )
                else:
                    lut_path = os.path.join(
                        lut_folder,
                        "{}_s{}c{}_{}_compress1.npy".format(lutName, 6, c, mode),
                    )
                key = "s{}c{}_{}_compress1".format(6, c, mode)
                if compressed_dimensions == "xy":
                    lut_arr = (
                        np.load(lut_path)
                        .reshape((-1, L * L, self.upscale * self.upscale))
                        .astype(np.float32)
                        / 127.0
                    )
                elif compressed_dimensions == "xyz":
                    lut_arr = (
                        np.load(lut_path)
                        .reshape((-1, L, self.upscale * self.upscale))
                        .astype(np.float32)
                        / 127.0
                    )
                elif compressed_dimensions == "xyzt":
                    lut_arr = (
                        np.load(lut_path)
                        .reshape((-1, self.upscale * self.upscale))
                        .astype(np.float32)
                        / 127.0
                    )
                else:
                ...
                if phase == "train":
                    lut_path = os.path.join(
                        lut_folder,
                        "{}_x{}_4bit_int8_s{}c{}_{}_compress2.npy".format(
                            lutName, upscale, 6, c, mode
                        ),
                    )
                else:
                    lut_path = os.path.join(
                        lut_folder,
                        "{}_s{}c{}_{}_compress2.npy".format(lutName, 6, c, mode),
                    )
        ...
        if phase == "train":
            lut_path = os.path.join(
                lut_folder,
                "{}_x{}_4bit_int8_s{}_channel.npy".format(lutName, upscale, 5),
            )
        else:
    ...
    def InterpTorchBatch_compress1_xyzt(
        self, weight_c1, upscale, out_c, mode, img_abcd, bd
    ):
        ...
        k0000 = self.ref2index[
            img_a1, img_b1 - img_a1, img_c1 - img_a1, img_d1 - img_a1
        ]
        k0001 = self.ref2index[
            img_a1, img_b1 - img_a1, img_c1 - img_a1, img_d2 - img_a1
        ]
        k0010 = self.ref2index[
            img_a1, img_b1 - img_a1, img_c2 - img_a1, img_d1 - img_a1
        ]
        k0011 = self.ref2index[
            img_a1, img_b1 - img_a1, img_c2 - img_a1, img_d2 - img_a1
        ]
        k0100 = self.ref2index[
            img_a1, img_b2 - img_a1, img_c1 - img_a1, img_d1 - img_a1
        ]
        k0101 = self.ref2index[
            img_a1, img_b2 - img_a1, img_c1 - img_a1, img_d2 - img_a1
        ]
        k0110 = self.ref2index[
            img_a1, img_b2 - img_a1, img_c2 - img_a1, img_d1 - img_a1
        ]
        k0111 = self.ref2index[
            img_a1, img_b2 - img_a1, img_c2 - img_a1, img_d2 - img_a1
        ]

        k1000 = self.ref2index[
            img_a2, img_b1 - img_a2, img_c1 - img_a2, img_d1 - img_a2
        ]
        k1001 = self.ref2index[
            img_a2, img_b1 - img_a2, img_c1 - img_a2, img_d2 - img_a2
        ]
        k1010 = self.ref2index[
            img_a2, img_b1 - img_a2, img_c2 - img_a2, img_d1 - img_a2
        ]
        k1011 = self.ref2index[
            img_a2, img_b1 - img_a2, img_c2 - img_a2, img_d2 - img_a2
        ]
        k1100 = self.ref2index[
            img_a2, img_b2 - img_a2, img_c1 - img_a2, img_d1 - img_a2
        ]
        k1101 = self.ref2index[
            img_a2, img_b2 - img_a2, img_c1 - img_a2, img_d2 - img_a2
        ]
        k1110 = self.ref2index[
            img_a2, img_b2 - img_a2, img_c2 - img_a2, img_d1 - img_a2
        ]
        k1111 = self.ref2index[
            img_a2, img_b2 - img_a2, img_c2 - img_a2, img_d2 - img_a2
        ]
        ...
        out = torch.zeros(
            (img_a1.shape[0], out_c * upscale * upscale), dtype=weight_c1.dtype
        ).to(device=weight_c1.device)
        sz = img_a1.shape[0]
        ...
        i3 = i = torch.all(
            torch.cat([~i1[:, None], ~i2[:, None], fab, fbc, fad], dim=1), dim=1
        )
        out[i] = (
            (q - fa[i]) * p0000[i]
            + (fa[i] - fd[i]) * p1000[i]
        ...
        i4 = i = torch.all(
            torch.cat([~i1[:, None], ~i2[:, None], ~i3[:, None], fab, fbc], dim=1),
            dim=1,
        )
        out[i] = (
            (q - fd[i]) * p0000[i]
        ...
        i6 = i = torch.all(
            torch.cat([~(fbc), ~i5[:, None], fab, fac, fcd], dim=1), dim=1
        )
        out[i] = (
            (q - fa[i]) * p0000[i]
            + (fa[i] - fc[i]) * p1000[i]
        ...
        i7 = i = torch.all(
            torch.cat([~(fbc), ~i5[:, None], ~i6[:, None], fab, fac, fad], dim=1), dim=1
        )
        out[i] = (
            (q - fa[i]) * p0000[i]
            + (fa[i] - fd[i]) * p1000[i]
        ...
        i8 = i = torch.all(
            torch.cat(
                [~(fbc), ~i5[:, None], ~i6[:, None], ~i7[:, None], fab, fac], dim=1
            ),
            dim=1,
        )
        out[i] = (
        ...
        i10 = i = torch.all(
            torch.cat([~(fbc), ~(fac), ~i9[:, None], fab, fad], dim=1), dim=1
        )  # c > a > d > b
        out[i] = (
            (q - fc[i]) * p0000[i]
            + (fc[i] - fa[i]) * p0010[i]
        ...
        i11 = i = torch.all(
            torch.cat([~(fbc), ~(fac), ~i9[:, None], ~i10[:, None], fab, fcd], dim=1),
            dim=1,
        )  # c > d > a > b
        out[i] = (
            (q - fc[i]) * p0000[i]
        ...
        i12 = i = torch.all(
            torch.cat(
                [~(fbc), ~(fac), ~i9[:, None], ~i10[:, None], ~i11[:, None], fab], dim=1
            ),
            dim=1,
        )
        out[i] = (
        ...
        i15 = i = torch.all(
            torch.cat([~(fab), ~i13[:, None], ~i14[:, None], fac, fbd], dim=1), dim=1
        )
        out[i] = (
            (q - fb[i]) * p0000[i]
            + (fb[i] - fd[i]) * p0100[i]
        ...
        i16 = i = torch.all(
            torch.cat(
                [~(fab), ~i13[:, None], ~i14[:, None], ~i15[:, None], fac], dim=1
            ),
            dim=1,
        )
        out[i] = (
        ...
        i18 = i = torch.all(
            torch.cat([~(fab), ~(fac), ~i17[:, None], fbc, fcd], dim=1), dim=1
        )
        out[i] = (
            (q - fb[i]) * p0000[i]
            + (fb[i] - fc[i]) * p0100[i]
        ...
        i19 = i = torch.all(
            torch.cat([~(fab), ~(fac), ~i17[:, None], ~i18[:, None], fbc, fbd], dim=1),
            dim=1,
        )
        out[i] = (
            (q - fb[i]) * p0000[i]
        ...
        i22 = i = torch.all(
            torch.cat([~(fab), ~(fac), ~(fbc), ~i21[:, None], fbd], dim=1), dim=1
        )
        out[i] = (
            (q - fc[i]) * p0000[i]
            + (fc[i] - fb[i]) * p0010[i]
        ...
        i23 = i = torch.all(
            torch.cat(
                [~(fab), ~(fac), ~(fbc), ~i21[:, None], ~i22[:, None], fcd], dim=1
            ),
            dim=1,
        )
        out[i] = (
    ...
    def unfold(K, P, x):
        ...
        x = x.reshape(B * C * (H - P) * (W - P), K, K)  # B*C*L,K,K
    ...
    def put_back(P, x, ori_shape):
        ...
        x = F.fold(x, ((H - P), (W - P)), 1, stride=1)
        return x

    def sample(
        self, sampler: AutoSample, img: torch.Tensor
        ...
        assert unfolded.shape[-2:] == (sampler.input_shape, sampler.input_shape), (
            f"Unexpected shape after unfold: {unfolded.shape}"
        )
        # unfolded: B*C*L,1,K,K
        sampled = sampler(unfolded)
        # sampled: B*C*L,1,2,2
        assert sampled.shape[:-2] == unfolded.shape[:-2] and sampled.shape[-2:] == (
            2,
            2,
        )
    ...
    def InterpTorchBatch(
        ...
        out_compress1, index_flag = self.InterpTorchBatch_compress1_xyzt(
            weight_c1, upscale, out_c, mode, (a, b, c, d), bd
        )
        ...
        out = torch.zeros(
            (
                ...
                img_a1.shape[2],
                img_a1.shape[3],
                out_c * upscale * upscale,
            ),
            dtype=weight_c2.dtype,
        ).to(device=weight_c2.device)
        ...
        if sz == 0:
            ...
            out_all = out_all.permute(0, 1, 4, 2, 5, 3, 6).reshape(
                (
                    img_a1.shape[0],
                    img_a1.shape[1] * out_c,
                    img_a1.shape[2] * upscale,
                    img_a1.shape[3] * upscale,
                )
            )
            return out_all
        ...
        assert self.inrange(
            0, 255 / L, img_a1, img_b1, img_c1, img_d1, img_a2, img_b2, img_c2, img_d2
        )
        key = (
            img_a1.flatten() * L * L * L
            + img_b1.flatten() * L * L
            + img_c1.flatten() * L
            + img_d1.flatten()
        )
        p0000 = weight_c2[
            ...
            + img_c1.flatten() * L
            + img_d1.flatten()
        ].reshape((sz, out_c * upscale * upscale))
        p0001 = weight_c2[
            img_a1.flatten() * L * L * L
            + img_b1.flatten() * L * L
            + img_c1.flatten() * L
    ...
    def InterpTorchBatch_channel(self, weight, out_c, img_in):
        ...
        img_a1, img_b1, img_c1, img_d1 = torch.chunk(
            torch.floor_divide(img_in, q).type(torch.int64), 4, 1
        )
