...
class BaseDNNets(nn.Module):
    def __init__(self, sample_size, nf=64, modes=["s", "d", "y"], stages=2):
        ...
        for s in range(stages):
            module = ConvBlock(in_c=1, modes=modes, nf=nf, sample_size=sample_size)
...
class BaseMuLUT_DFC(nn.Module):
            ...
            for c in range(c_in):
                for mode in modes:
                    lut_path = os.path.join(
                        lut_folder,
                        "{}_s{}_c{}_{}_compress1.npy".format(
                            lutName, str(stage), str(c), mode
                        ),
                    )
                    ...
                    if self.compression == "xy":
                        ...
                        lut_arr = (
                            np.load(lut_path).reshape(-1, L, c_in).astype(np.float32)
                            / 127.0
                        )
                    elif self.compression == "xyzt":
                        lut_arr = (
                            np.load(lut_path).reshape(-1, c_in).astype(np.float32)
                            / 127.0
                        )
                    else:
                        raise ValueError(
                            "compression {} not implemented.".format(self.compression)
                        )
                    ...
                    lut_path = os.path.join(
                        lut_folder,
                        "{}_s{}_c{}_{}_compress2.npy".format(
                            lutName, str(stage), str(c), mode
                        ),
                    )
                    key = "s{}_c{}_{}_compress2".format(str(stage), str(c), mode)
                    lut_arr = (
                        np.load(lut_path).reshape(-1, c_in).astype(np.float32) / 127.0
                    )
                    self.register_parameter(
                        name="weight_" + key,
                        param=torch.nn.Parameter(torch.Tensor(lut_arr)),
    ...
    def get_1d_index(i, j, N, L, diag):
        ...
        if 2 * d < L + 3:
            ...
            k[index2] = (
                (diag - 1) * i[index2] + j[index2] + ((5 - diag) * (1 + diag)) // 8 - 1
            )
            k[index3] = (
                N
                - torch.floor_divide(
                    (L - i[index3] - 1) * (L - i[index3] - 1 + diag), 2
                ).type(torch.int64)
                - L
                + j[index3]
            )
    ...
    def InterpTorchBatch_compress1(self, weight_c1, mode, img_in, bd):
        ...
        if mode == "s":
            ...
            img_a1 = torch.floor_divide(img_in[:, :, 0 : 0 + h, 0 : 0 + w], q).type(
                torch.int64
            )
            img_b1 = torch.floor_divide(img_in[:, :, 0 : 0 + h, 1 : 1 + w], q).type(
                torch.int64
            )
            img_c1 = torch.floor_divide(img_in[:, :, 1 : 1 + h, 0 : 0 + w], q).type(
                torch.int64
            )
            img_d1 = torch.floor_divide(img_in[:, :, 1 : 1 + h, 1 : 1 + w], q).type(
                torch.int64
            )
            ...
            img_b1 = torch.floor_divide(img_in[:, :, 0 : 0 + h, 2 : 2 + w], q).type(
                torch.int64
            )
            img_c1 = torch.floor_divide(img_in[:, :, 2 : 2 + h, 0 : 0 + w], q).type(
                torch.int64
            )
            img_d1 = torch.floor_divide(img_in[:, :, 2 : 2 + h, 2 : 2 + w], q).type(
                torch.int64
            )
            ...
            img_b1 = torch.floor_divide(img_in[:, :, 1 : 1 + h, 1 : 1 + w], q).type(
                torch.int64
            )
            img_c1 = torch.floor_divide(img_in[:, :, 1 : 1 + h, 2 : 2 + w], q).type(
                torch.int64
            )
            img_d1 = torch.floor_divide(img_in[:, :, 2 : 2 + h, 1 : 1 + w], q).type(
                torch.int64
            )
        ...
        out = torch.zeros((img_a1.shape[0], self.c_in, 1), dtype=weight_c1.dtype).to(
    ...
    def InterpTorchBatch_compress1_xyzt(self, weight_c1, out_c, mode, img_in, bd):
        ...
        out = torch.zeros((img_a1.shape[0], out_c), dtype=weight_c1.dtype).to(
            device=weight_c1.device
        )
        sz = img_a1.shape[0]
        out = out.reshape(sz, -1)
    ...
    def InterpTorchBatch(self, weight_c1, weight_c2, mode, img_in, bd):
        ...
        if self.compression == "xy":
            out_compress1, index_flag = self.InterpTorchBatch_compress1(
                weight_c1, mode, img_in, bd
            )
        elif self.compression == "xyz":
            out_compress1, index_flag = None, None
        else:
            out_compress1, index_flag = self.InterpTorchBatch_compress1_xyzt(
                weight_c1, self.c_in, mode, img_in, bd
            )
        ...
        if sz == 0:
            ...
            out_all = out_all.permute(0, 1, 4, 2, 3).reshape(
...
class SPF_LUT_DFC(nn.Module):
    ...
    def InterpTorchBatch(self, weight_c1, weight_c2, out_c, mode, img_in, bd):
        ...
        out_compress1, index_flag = self.InterpTorchBatch_compress1_xyzt(
            weight_c1, out_c, mode, img_in, bd
        )
        index_flag = index_flag.flatten()
