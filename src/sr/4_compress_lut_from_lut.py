...
def compress_MuLUT_from_lut(opt):
    ...
    np.save(
        os.path.join(save_dir, "ref2index_{}{}i{}.npy".format(opt.cd, opt.dw, opt.si)),
        ref2index,
    )
    ...
    for s in range(stages):
        for mode in modes:
            lut_path = os.path.join(
                exp_dir, "{}_s{}_{}.npy".format(opt.lutName, s + 1, mode)
            )
            lut = np.load(lut_path)
            _, C = lut.shape

            if compression_type == "xy":
                lut_c1 = lut.reshape((L * L, L, L, -1))[
                    index_compress, :, :, :
                ].reshape((-1, C))
            elif compression_type == "xyz":
                lut_c1 = lut.reshape((L * L * L, L, -1))[index_compress, :, :].reshape(
                    (-1, C)
                )
            else:
            ...
            save_path = os.path.join(
                save_dir, "{}_s{}_{}_compress1.npy".format(opt.lutName, s + 1, mode)
            )
            np.save(save_path, lut_c1)
            print("Resulting LUT size: ", lut_c1.shape, "Saved to", save_path)

            save_path = os.path.join(
                save_dir, "{}_s{}_{}_compress2.npy".format(opt.lutName, s + 1, mode)
            )
...
def compress_SPFLUT_from_lut(opt):
    ...
    for s in range(6):
        if s + 1 == 5:
            lut_path = os.path.join(
                exp_dir, "{}_s{}_channel.npy".format(opt.lutName, s + 1)
            )
            lut = np.load(lut_path)
            save_path = os.path.join(
                save_dir, "{}_s{}_channel.npy".format(opt.lutName, s + 1)
            )
        ...
        for mode in modes:
            if s + 1 == 6:
                for c in range(4):
                    lut_path = os.path.join(
                        exp_dir, "{}_s{}c{}_{}.npy".format(opt.lutName, s + 1, c, mode)
                    )
                    lut = np.load(lut_path)
                    _, C = lut.shape
                ...
                lut_path = os.path.join(
                    exp_dir, "{}_s{}c0_{}.npy".format(opt.lutName, s + 1, mode)
                )
                lut = np.load(lut_path)
                _, C = lut.shape
                ...
                save_path = os.path.join(
                    save_dir,
                    "{}_s{}c0_{}_compress1.npy".format(opt.lutName, s + 1, mode),
                )
                np.save(save_path, lut_c1)
                print("Resulting LUT size: ", lut_c1.shape, "Saved to", save_path)
                ...
                save_path = os.path.join(
                    save_dir,
                    "{}_s{}c0_{}_compress2.npy".format(opt.lutName, s + 1, mode),
                )
                np.save(save_path, lut_c2)
                print("Resulting LUT size: ", lut_c2.shape, "Saved to", save_path)
