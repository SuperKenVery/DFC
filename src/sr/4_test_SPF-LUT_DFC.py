...
from ..common.utils import PSNR, _rgb2ycbcr, cal_ssim, modcrop

# compression_type = 'xyzt'


def InterpTorchBatch_compress_xy(
    weight, img_in, h, w, interval, rot, d, upscale=4, out_c=1, mode="s", ref2index=None
):
    q = 2**interval  # 16
    L = 2 ** (8 - interval) + 1  # 17
...
def InterpTorchBatch_compress_xyz(
    weight, img_in, h, w, interval, rot, d, upscale=4, out_c=1, mode="s", ref2index=None
):
    q = 2**interval  # 16
    L = 2 ** (8 - interval) + 1  # 17
...
def InterpTorchBatch_compress_xyzt(
    weight, img_in, h, w, interval, rot, d, upscale=4, out_c=1, mode="s", ref2index=None
):
    q = 2**interval  # 16
    L = 2 ** (8 - interval) + 1  # 17
...
def FourSimplexInterpFaster_compress(
    weight_c1,
    weight_c2,
    img_in,
    h,
    ...
    if compression_type == "xy":
        out_compress1, index_flag = InterpTorchBatch_compress_xy(
            weight_c1,
            img_in,
            h,
            w,
        ...
        out_compress1, index_flag = InterpTorchBatch_compress_xyz(
            weight_c1,
            img_in,
            h,
            w,
        ...
        out_compress1, index_flag = InterpTorchBatch_compress_xyzt(
            weight_c1,
            img_in,
            h,
            w,
...
def InterpTorchBatch_channel(weight, out_c, img_in, interval):
    ...
    p0000 = weight[
        img_a1.flatten(), img_b1.flatten(), img_c1.flatten(), img_d1.flatten()
    ].reshape(
        (img_a1.shape[0], img_a1.shape[1], img_a1.shape[2], img_a1.shape[3], out_c)
    )
    p0001 = weight[
        img_a1.flatten(), img_b1.flatten(), img_c1.flatten(), img_d2.flatten()
    ].reshape(
        (img_a1.shape[0], img_a1.shape[1], img_a1.shape[2], img_a1.shape[3], out_c)
    )
    p0010 = weight[
        img_a1.flatten(), img_b1.flatten(), img_c2.flatten(), img_d1.flatten()
    ].reshape(
        (img_a1.shape[0], img_a1.shape[1], img_a1.shape[2], img_a1.shape[3], out_c)
    )
    p0011 = weight[
        img_a1.flatten(), img_b1.flatten(), img_c2.flatten(), img_d2.flatten()
    ].reshape(
        (img_a1.shape[0], img_a1.shape[1], img_a1.shape[2], img_a1.shape[3], out_c)
    )
    p0100 = weight[
        img_a1.flatten(), img_b2.flatten(), img_c1.flatten(), img_d1.flatten()
    ].reshape(
        (img_a1.shape[0], img_a1.shape[1], img_a1.shape[2], img_a1.shape[3], out_c)
    )
    p0101 = weight[
        img_a1.flatten(), img_b2.flatten(), img_c1.flatten(), img_d2.flatten()
    ].reshape(
        (img_a1.shape[0], img_a1.shape[1], img_a1.shape[2], img_a1.shape[3], out_c)
    )
    p0110 = weight[
        img_a1.flatten(), img_b2.flatten(), img_c2.flatten(), img_d1.flatten()
    ].reshape(
        (img_a1.shape[0], img_a1.shape[1], img_a1.shape[2], img_a1.shape[3], out_c)
    )
    p0111 = weight[
        img_a1.flatten(), img_b2.flatten(), img_c2.flatten(), img_d2.flatten()
    ].reshape(
        (img_a1.shape[0], img_a1.shape[1], img_a1.shape[2], img_a1.shape[3], out_c)
    )

    p1000 = weight[
        img_a2.flatten(), img_b1.flatten(), img_c1.flatten(), img_d1.flatten()
    ].reshape(
        (img_a1.shape[0], img_a1.shape[1], img_a1.shape[2], img_a1.shape[3], out_c)
    )
    p1001 = weight[
        img_a2.flatten(), img_b1.flatten(), img_c1.flatten(), img_d2.flatten()
    ].reshape(
        (img_a1.shape[0], img_a1.shape[1], img_a1.shape[2], img_a1.shape[3], out_c)
    )
    p1010 = weight[
        img_a2.flatten(), img_b1.flatten(), img_c2.flatten(), img_d1.flatten()
    ].reshape(
        (img_a1.shape[0], img_a1.shape[1], img_a1.shape[2], img_a1.shape[3], out_c)
    )
    p1011 = weight[
        img_a2.flatten(), img_b1.flatten(), img_c2.flatten(), img_d2.flatten()
    ].reshape(
        (img_a1.shape[0], img_a1.shape[1], img_a1.shape[2], img_a1.shape[3], out_c)
    )
    p1100 = weight[
        img_a2.flatten(), img_b2.flatten(), img_c1.flatten(), img_d1.flatten()
    ].reshape(
        (img_a1.shape[0], img_a1.shape[1], img_a1.shape[2], img_a1.shape[3], out_c)
    )
    p1101 = weight[
        img_a2.flatten(), img_b2.flatten(), img_c1.flatten(), img_d2.flatten()
    ].reshape(
        (img_a1.shape[0], img_a1.shape[1], img_a1.shape[2], img_a1.shape[3], out_c)
    )
    p1110 = weight[
        img_a2.flatten(), img_b2.flatten(), img_c2.flatten(), img_d1.flatten()
    ].reshape(
        (img_a1.shape[0], img_a1.shape[1], img_a1.shape[2], img_a1.shape[3], out_c)
    )
    p1111 = weight[
        img_a2.flatten(), img_b2.flatten(), img_c2.flatten(), img_d2.flatten()
    ].reshape(
        (img_a1.shape[0], img_a1.shape[1], img_a1.shape[2], img_a1.shape[3], out_c)
    )
...
class eltr:
    def __init__(self, dataset, opt, lutDict, ref2index=None):
        ...
        result_path = os.path.join(
            opt.resultRoot, exp_name, dataset, "X{}".format(opt.scale)
        )
    ...
    def run(self, num_worker=24):
        ...
        print(
            "Dataset {} | AVG LUT PSNR: {:.2f} SSIM: {:.4f}".format(
                dataset,
                np.mean(np.asarray(psnr_ssim_s)[:, 0]),
    ...
    def _worker(self, i):
        ...
        img_lr = np.array(
            Image.open(
                os.path.join(
                    self.opt.testDir,
                    self.dataset,
                    "LR_bicubic/X{}".format(self.opt.scale),
                    self.files[i],
                )
            )
        ).astype(np.float32)
        ...
        img_gt = np.array(
            Image.open(
                os.path.join(self.opt.testDir, self.dataset, "HR", self.files[i])
            )
        )
        ...
        for s in range(4):
            ...
            for mode in self.opt.modes:
                ...
                for r in [0, 1, 2, 3]:
                    ...
                    img_in = np.pad(
                        img_lr_rot, (pad, pad, (0, 0)), mode="edge"
                    ).transpose((2, 0, 1))  # (3, H, W)
                    img_in = img_in[:, np.newaxis, :, :]  # (3, 1, H, W)
                    pred += FourSimplexInterpFaster_compress(
                        self.lutDict[key1],
                        self.lutDict[key2],
                        img_in,
                        h,
...
if __name__ == "__main__":
        ...
        for c in range(4):
            ...
            if compression_type == "xy":
                lutDict[key] = (
                    np.load(lut_path)
                    .astype(np.float32)
                    .reshape((-1, L, L, opt.scale**2))
                )
            elif compression_type == "xyz":
                lutDict[key] = (
                    np.load(lut_path).astype(np.float32).reshape((-1, L, opt.scale**2))
                )
            else:
    ...
    ref2index = np.load(
        os.path.join(opt.expDir, "ref2index_{}{}i{}.npy".format(opt.cd, opt.dw, opt.si))
    )
    # ref2index = None
