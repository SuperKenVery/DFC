...
from .data import DBBenchmark, Provider
...
if __name__ == "__main__":
    ...
    # Training dataset
    train_iter = Provider(
        opt.batchSize, opt.workerNum, opt.qf, opt.trainDir, opt.cropSize
    )
            ...
            for c in range(c_in):
                for mode in modes:
                    ft_lut_path = os.path.join(
                        opt.expDir,
                        "LUT_ft_qf{}_{}bit_int8_s{}_c{}_{}_compress1.npy".format(
                            opt.qf, opt.interval, str(stage), str(c), mode
                        ),
                    )
                    lut_weight = np.round(
                        np.clip(
                    ...
                    ft_lut_path = os.path.join(
                        opt.expDir,
                        "LUT_ft_qf{}_{}bit_int8_s{}_c{}_{}_compress2.npy".format(
                            opt.qf, opt.interval, str(stage), str(c), mode
                        ),
                    )
                    lut_weight = np.round(
