...
from ..common.utils import PSNR, _rgb2ycbcr, cal_ssim
...
def valid_steps(opt, model_G, valid, exp_name):
    with torch.no_grad():
        ...
        for key in tqdm(range(len(valid))):
            input_im = valid.lr_ims[
                "/gdata/liyl" + valid.blur_list[key][7:].replace("\\", "/")
            ]
            lb = valid.hr_ims[
                "/gdata/liyl" + valid.sharp_list[key][7:].replace("\\", "/")
            ]
            im = torch.Tensor(
        ...
        print(
            "Dataset {} | AVG Val PSNR: {} SSIM: {}".format(
                "GoPro", np.mean(np.asarray(psnrs)), np.mean(np.asarray(ssims))
            )
        )
