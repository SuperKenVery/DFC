...
from .data import GoProTest, Provider
...
def valid_steps(model_G, valid, opt, iter):
    ...
    with torch.no_grad():
        ...
        for key in range(len(valid)):
            ...
            im = torch.Tensor(
                np.expand_dims(np.transpose(input_im, [2, 0, 1]), 0).astype(np.float32)
                / 255.0
            ).cuda()
            ...
            if iter < 10000:  # save input and gt at start
                ...
                Image.fromarray(input_img[:, :]).save(
                    os.path.join(
                        ...
                        valid.blur_list[key]
                        .replace("/", "_")
                        .replace(".png", "_input.png"),
                    )
                )
                Image.fromarray(lb[:, :].astype(np.uint8)).save(
                    os.path.join(
                        result_path,
                        valid.sharp_list[key]
                        .replace("/", "_")
                        .replace(".png", "_gt.png"),
                    )
                )
            ...
            Image.fromarray(pred[:, :]).save(
                os.path.join(
                    result_path,
                    valid.sharp_list[key].replace("/", "_").replace(".png", "_net.png"),
                )
            )
        ...
        writer.scalar_summary(
            "PSNR_valid/{}".format("GoPro"), np.mean(np.asarray(psnrs)), iter
        )
            ...
            for c in range(c_in):
                for mode in modes:
                    ft_lut_path = os.path.join(
                        opt.expDir,
                        "LUT_ft_{}bit_int8_s{}_c{}_{}_compress1.npy".format(
                            opt.interval, str(stage), str(c), mode
                        ),
                    )
                    lut_weight = np.round(
                        np.clip(
                    ...
                    ft_lut_path = os.path.join(
                        opt.expDir,
                        "LUT_ft_{}bit_int8_s{}_c{}_{}_compress2.npy".format(
                            opt.interval, str(stage), str(c), mode
                        ),
                    )
                    lut_weight = np.round(
