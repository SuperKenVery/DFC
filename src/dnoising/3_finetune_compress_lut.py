...
from ..common.utils import PSNR, cPSNR, logger_info
from ..common.Writer import Logger
from . import model as Model
from .data import DNBenchmark, Provider
...
def SaveCheckpoint(model_G, opt_G, opt, i, best=False):
    ...
    torch.save(
        model_G, os.path.join(opt.expDir, "Model_{:06d}{}.pth".format(i, str_best))
    )
        ...
        for i in range(len(datasets)):
            ...
            logger.info(
                "Iter {} | Dataset {} | AVG Val PSNR: {:02f}".format(
                    iter, datasets[i], np.mean(np.asarray(psnrs))
                )
            )
            writer.scalar_summary(
                "PSNR_valid/{}".format(datasets[i]), np.mean(np.asarray(psnrs)), iter
            )
...
if __name__ == "__main__":
    ...
    # Training dataset
    train_iter = Provider(
        opt.batchSize, opt.workerNum, opt.sigma, opt.trainDir, opt.cropSize
    )
            ...
            for c in range(c_in):
                for mode in modes:
                    ft_lut_path = os.path.join(
                        opt.expDir,
                        "LUT_ft_sigma{}_{}bit_int8_s{}_c{}_{}_compress1.npy".format(
                            opt.sigma, opt.interval, str(stage), str(c), mode
                        ),
                    )
                    lut_weight = np.round(
                        np.clip(
                            getattr(
                                model_G,
                                "weight_s{}_c{}_{}_compress1".format(
                                    str(s + 1), str(c), mode
                                ),
                            )
                            .cpu()
                    ...
                    ft_lut_path = os.path.join(
                        opt.expDir,
                        "LUT_ft_sigma{}_{}bit_int8_s{}_c{}_{}_compress2.npy".format(
                            opt.sigma, opt.interval, str(stage), str(c), mode
                        ),
                    )
                    lut_weight = np.round(
                        np.clip(
                            getattr(
                                model_G,
                                "weight_s{}_c{}_{}_compress2".format(
                                    str(s + 1), str(c), mode
                                ),
                            )
                            .cpu()
