...
from ..common.utils import PSNR, cPSNR
    ...
    with torch.no_grad():
        ...
        for i in range(len(datasets)):
            ...
            result_path = os.path.join(
                opt.resultRoot, exp_name, datasets[i], "sigma{}".format(opt.sigma)
            )
            ...
            print(
                "Dataset {} | AVG Val PSNR: {:02f}".format(
                    datasets[i], np.mean(np.asarray(psnrs))
                )
            )
